...
const u32* CGameGraphBuilder::validate_cross_table(const CGameGraph& pack, const GameGraph::_LEVEL_ID pack_level_id, string256& reason)
{
	...
	u32 level_vertices = 0;
	for (u32 i = 0, n = pack.header().vertex_count(); i < n; ++i)
		level_vertices += u32(pack.vertex(i)->level_id() == pack_level_id);
	if (table_header->game_vertex_count() != level_vertices)
	{
		xr_sprintf(reason, "cross table is for %d graph vertices, the level has %d", table_header->game_vertex_count(), level_vertices);
		return nullptr;
	}
...
bool CGameGraphBuilder::append_level(const CGameGraph& pack, const GameGraph::_LEVEL_ID pack_level_id, const Fvector& offset, xr_vector<u32>& vertex_map, string256& reason)
{
	const GameGraph::SLevel& source = pack.header().level(pack_level_id);
	if (m_header.m_levels.find(source.id()) != m_header.m_levels.end())
	{
		xr_sprintf(reason, "level id %d is already used by '%s'", source.id(), *m_header.level(source.id()).name());
		return false;
	}
