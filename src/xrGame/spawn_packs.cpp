...
namespace
{
	...
	// [levelNN] section of level_name listed under game.ltx [levels]: false when absent; id_mismatch when its id differs.
	bool listed_level(const LPCSTR level_name, const GameGraph::_LEVEL_ID id, Fvector& offset, u32& listed_id, bool& id_mismatch)
	{
		id_mismatch = false;
	...
	bool pack_object_present(const CSpawnPacks::SPack& pack, u32 pack_vertex_id, const SPAWN_GRAPH& spawns, const NAME_INDEX& known_index, const CGameGraph& graph, const LPCSTR name, const Fvector& position, ALife::_SPAWN_ID& match)
	{
		...
		if (std::find(pack.levels.begin(), pack.levels.end(), pack_level_id) != pack.levels.end())
			return false;
...
u32 CSpawnPacks::append_levels(CGameGraphBuilder& builder, float tolerance)
{
	...
	for (auto* entry : m_packs)
	{
		...
		for (; I != E; ++I)
		{
			...
			u32 listed_id = 0;
			bool id_mismatch;
			if (!listed_level(level_name, level.id(), offset, listed_id, id_mismatch))
			{
			...
			if (id_mismatch)
			{
				Msg("! [spawn_overlays] pack %s: level '%s' has id %d in the pack but %d in its game.ltx [levelNN] section, skipped", file_name, level_name, level.id(), listed_id);
				++pack.missing_levels;
				continue;
			}

			const u32 before = builder.vertex_count();
			if (!builder.append_level(*pack.graph, level.id(), offset, pack.vertex_map, reason))
			{
				Msg("! [spawn_overlays] pack %s: level '%s': %s, skipped", file_name, level_name, reason);
				++pack.missing_levels;
				continue;
			}
			pack.levels.push_back(level.id());
			++changed;
			Msg("* [spawn_overlays] pack %s: level '%s' (id %d) +%d vertices at [%.1f,%.1f,%.1f]", file_name, level_name, level.id(), builder.vertex_count() - before, VPUSH(offset));
		}

		if (pack.levels.empty() && pack.substituted.empty())
			continue;
...
u32 CSpawnPacks::add_objects(STemplates& templates, const CGameGraph& graph)
{
	...
	for (auto* entry : m_packs)
	{
		...
		for (IReader* vertex = vertices->open_chunk_iterator(vertex_id); vertex; vertex = vertices->open_chunk_iterator(vertex_id, vertex))
		{
			...
			if (std::find(pack.levels.begin(), pack.levels.end(), graph.vertex(dynamic_object->m_tGraphID)->level_id()) == pack.levels.end())
...
void CSpawnPacks::finish(const xrGUID& base_guid, xrGUID& result) const
{
	...
	for (auto* entry : m_packs)
	{
		SPack& pack = *entry;
		const LPCSTR file_name = *pack.file_name;
		const bool changes_levels = !pack.levels.empty() || !pack.substituted.empty();
