...
public:
	...
	// Appends pack level pack_level_id (name, id, section, guid from the pack header, offset as given) with its
	// vertices, death points and cross table. vertex_map is sized to the pack and receives the new ids of the
	// appended vertices (u32(-1) elsewhere); edges are added by append_edges. Needs the pack and vertex_map alive
	// until build() called.
	bool append_level(const CGameGraph& pack, GameGraph::_LEVEL_ID pack_level_id, const Fvector& offset, xr_vector<u32>& vertex_map, string256& reason);
